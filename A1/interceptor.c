...
{
	//if a process is exitting, delete it from all intercepted syscalls
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	//log only if process is being monitored or if pid:0 (root) is in the monitored list (i.e monitored=2)
	...
	if (syscall < NR_syscalls && syscall > 0){
		...
		switch (cmd){
			case REQUEST_SYSCALL_INTERCEPT :
				...
				//proceed with command 1
				
				//save replaced system call
				table[syscall].f = sys_call_table[syscall];
				table[syscall].intercepted = 1;
				
				//lock the system call so that no other proccesses can alter the system call table
				spin_lock(&sys_call_table_lock);
				set_addr_rw((unsigned long)sys_call_table);
				
				//Rewrite this system call to point to our interceptor
				sys_call_table[syscall] = &interceptor;
				
				//Set to read only and unlock spinlock
			...
			case REQUEST_SYSCALL_RELEASE :
				...
				//check to see if the system call is even being intercepted
				if (table[syscall].intercepted==1){

					//alter meta data to match a released syscall
					spin_lock(&my_table_lock);
					...
					//Rewrite table to restore the system call.
			...
			case REQUEST_START_MONITORING :
				...
				if((pid > 0 && pid_task(find_vpid(pid), PIDTYPE_PID)!=NULL)|| pid == 0){
					...
					if (current_uid() !=0){
							...
							//check if process to be monitored is owned by calling process
							if (check_pids_same_owner(current->pid,pid) != 0){
								return -EPERM;
							}
						}

					//cannot request a monitor if the system call isn't being intercepted	
					if (table[syscall].intercepted != 1){
						return -EINVAL;
					}

					//check if pid is already being monitored for this system call
					if (check_pid_monitored(syscall,pid) != 0){
						return -EBUSY;
					}

					//passed checks -> proceed with command 3
					spin_lock(&my_table_lock);
					if (add_pid_sysc(pid,syscall) != 0){ //may be out of memory
						spin_unlock(&my_table_lock);//MUST UNLOCK SPIN LOCK BEFORE RETURNING
						return -ENOMEM;
					}
					if (pid==0){
			...
			case REQUEST_STOP_MONITORING :
				...
				if((pid > 0 && pid_task(find_vpid(pid), PIDTYPE_PID)!=NULL)|| pid == 0){
					...
					//check to see if pid is even being monitored
					...
					if (pid==0){
					 	destroy_list(syscall); //stop monitoring for all pids by destroying the list 
					 }
...
static int init_function(void) {
	...
	//initializing table fields to prevent memory problems
	while(ind < NR_syscalls){
		table[ind].f = NULL;
		table[ind].intercepted=0;
		table[ind].monitored=0;
		table[ind].listcount=0;
		INIT_LIST_HEAD(&(table[ind].my_list)); 
		ind++;
	}
...
{  
	//before exiting stop monitoring all pids and release every system call
	int ind=0;
	while(ind < NR_syscalls){
		if (table[ind].intercepted==1){
			//release all intercepted calls
			spin_lock(&my_table_lock);
			destroy_list(ind);
			table[ind].intercepted=0;
			spin_unlock(&my_table_lock);
			...
			spin_lock(&sys_call_table_lock);
			set_addr_rw((unsigned long)sys_call_table);
			sys_call_table[ind] = table[ind].f;
			set_addr_ro((unsigned long)sys_call_table);
			spin_unlock(&sys_call_table_lock);
		}
		ind++;
