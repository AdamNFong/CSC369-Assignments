...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if (victim_page->frame & PG_DIRTY){//been modified -> swap out
			int generated_swap_off = 0;
			generated_swap_off = swap_pageout (frame, victim_page->swap_off);
			victim_page -> swap_off = generated_swap_off;//now on swap

			//no longer valid because it is now on swap
...
char *find_physpage(addr_t vaddr, char type) {
	...
	uintptr_t pdEnt = pgdir[idx].pde;

	if (pdEnt == 0){//detect first usage
	...
	pgtbl_entry_t *pgtbl = (pgtbl_entry_t *)(pgdir[idx].pde - 1); //remove valid bit	 

	p = &pgtbl[PGTBL_INDEX (vaddr)]; //p now points to page table entry in second level


	// Check if p is valid or not, on swap or not, and handle appropriately
	if (p->frame & PG_VALID){//if valid, i.e a hit
		hit_count = hit_count + 1; 
	}else { //not valid i.e not in memory, may or may not be on swap
		miss_count = miss_count + 1;

		//need to allocate frame in both cases
		int all_frame = allocate_frame(p);

		if (!(p->frame & PG_ONSWAP)){//not on swap
			//initialize frame
			init_frame(all_frame, vaddr);
			p->frame = all_frame << PAGE_SHIFT;	
			p->frame = p->frame | PG_DIRTY;
			coremap[all_frame].vaddr = vaddr; //store the vaddr to use later in opt

		}else{//on swap, swapin (page frame #, swap offset)
			if (swap_pagein(all_frame, p->swap_off)!=0){
				perror("Error swapping in page");
			}
			p->frame = all_frame << PAGE_SHIFT;
			//shouldn't NEED to do these since all_frame will shifted with zeros but for peace of mind	
			p->frame = p->frame & ~PG_DIRTY;
	...
	//frame is now valid and referenced to.
